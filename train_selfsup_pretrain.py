...
def get_args_parser():
    ...
    parser.add_argument('--batch_size_per_gpu', default=32, type=int,
        help='Per-GPU batch-size : number of distinct images loaded on one GPU.  128')
    ...
    parser.add_argument('--data_path', default='D:/project_file/python/dataset', type=str,
    ...
    parser.add_argument('--saveckp_freq', default=20, type=int, help='Save checkpoint every x epochs.40')
...
def pretrain(args):
    ...
    if utils.is_main_process():
        if args.use_wandb:  # Log main process stats
            run = wandb.init(config=args, project="pretrain", entity=WANDB_USER)
        else:
            run = None
    ...
    # ============ preparing data ... ============
    # Get transforms for data augmentation for pretraining (slightly extended compared to original iBOT)
...
class iBOTLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, student_local_cls, student_mask, epoch):
        ...
        teacher_cls_c = F.softmax((teacher_cls - self.center) / temp, dim=-1)  # 最后8192
        teacher_cls_c = teacher_cls_c.detach().chunk(self.ngcrops)
        teacher_patch_c = F.softmax((teacher_patch - self.center2) / temp2, dim=-1)  # 8192
        ...
        for q in range(len(teacher_cls_c)):
            for v in range(len(student_cls_c)):
                if v == q:  # q和v是不同view（数据增强）的索引,共两个
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('pretrain', parents=[get_args_parser()])
    ...
    if args.output_dir == '':
        args.output_dir = os.path.join(utils.get_base_path(), 'pretrain_saves')
